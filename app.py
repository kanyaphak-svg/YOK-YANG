...
try:
    ...
    st.error("ไม่พบไฟล์ yolov8n-pose.pt ใน GitHub กรุณาอัปโหลดไฟล์นี้ด้วยครับ")
    st.stop()
    ...
    def recv(self, frame):
        ...
        try:
            if results[0].keypoints.data.shape[1] > 0:
                ...
                # เช็คว่าเห็นคนชัดไหม (ไหล่และศอก)
                if keypoints[6][2] > 0.5 and keypoints[8][2] > 0.5:
                    ...
                    cv2.putText(img, self.feedback, (20, 45), cv2.FONT_HERSHEY_SIMPLEX, 1.0, (255, 255, 255), 2)
                    # Stats Box
                    cv2.rectangle(img, (0, 400), (200, 480), (0, 0, 0), -1)
                    cv2.putText(img, f"REPS: {self.counter}", (10, 440), cv2.FONT_HERSHEY_SIMPLEX, 0.8, (255, 255, 255), 2)
                    cv2.putText(img, f"SETS: {self.set_count}", (10, 470), cv2.FONT_HERSHEY_SIMPLEX, 0.6, (0, 255, 0), 2)
...
st.write("---")
st.markdown("### สถานะการใช้งาน:")
st.markdown("- 🟩 **สีเขียว:** ท่าถูกต้อง")
st.markdown("- 🟥 **สีแดง:** ท่าผิด (อ่านคำแนะนำบนหน้าจอ)")

webrtc_streamer(
    key="fitness-coach-final",
